...
测试覆盖：全部最低项、超额积分、关闭前不罚、冻结保连胜、周补卡上限、罚金累计和兑换、里程碑、人工审核延迟、覆盖后重放、设置快照、跨午夜、春秋 DST、类别去重、BQ 不同日两阶段及毕业后复述、看剧集数必须刚好及旧训练营升级、角色权限、输入校验、重复动作、模型错误回退；名字进入与名字锁定、裁判密钥、伪造 / 篡改 cookie、改名后旧会话失效、跨站请求。
