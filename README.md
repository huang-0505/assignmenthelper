...
**隐私**：摄像头只在点击「开启摄像头」、看过说明之后才请求；页面上始终显示「摄像头开启中」，一键即可关闭（关闭即结束本场）。视频只在本机分析。第一次开启 AI 查岗前必须同意：画面会发送给 Google Gemini API；免费额度下 Google 会用这些内容改进产品，可能有人工审核员查看，且 Google 建议不要在免费服务中提交个人信息。给 API key 开通付费后，内容不会用于改进产品。不同意时只用本机检测。服务器不保存判断用的画面；只有她打开「违规截图给裁判看」并被记提醒或违规时，才把一张缩小的 JPEG 存进私有 Storage 桶（服务端自动创建，只有经过验证的玩家和裁判能通过 `/api/study/media` 读取）。她可以随时删除全部截图。
