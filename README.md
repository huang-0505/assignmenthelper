...
- 记录投递申请时可以顺手粘贴职位描述（JD，最多 12000 字）。JD 正文存进私有存储桶 `job-docs`（服务端自动创建，只有玩家和裁判能通过 `/api/jd` 读取），训练记录里只留一个大小标记，所以每次打开页面不会被 JD 拖慢。记录列表、打卡日历的当日记录和裁判的今日实况里都能点开「JD」查看；删除这条记录时，JD 一起删除。
